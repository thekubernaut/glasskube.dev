...
deliver their applications to complex and many times restricted on-premises environments.
...
1. **Distr** is the latest Software Delivery Platform built by Glasskube that helps software vendors serve a wide variety of use-cases.
...
Distr provides agents for guided Docker, Kubernetes, VPC, BYOC and on-premises installations,
...
2. **Replicated** is a mature software distribution platform that facilitates the delivery of applications to on-premises, VPC, and air gapped environments
by wrapping your software in an installer ISVs can repackage their software. Replicated also provides license management capabilities, pre-flight checks,
and compatibility testing.

## Software Delivery Platform Users (ISVs) vs. End Customers
...
Software distribution platforms primarily serve **independent software vendors ([ISVs](/glossary/isv-meaning/))**.
While ISVs are the direct users of these platforms, the ultimate goal is to bridge the gap between the ISV and their **end-customers**.
...
- **Independent Software Vendors:** These are the primary users of software distribution platforms.
They leverage the platform's tools and features to package, distribute, and manage their software in complex environments.
- **End-Customers:** In the context of on-prem software delivery, they are the party which software is delivered to.
They interact with software distribution platforms indirectly and if done well, they need not even know they are interacting with one at all.

## Distr as Replicated alternative

Distr is an open source alternative to Replicated, offering a modern and flexible approach to software distribution.

1. Distr provides **one platform** for on-prem, shared-responsibility, and self-service software distribution.
...
Distr has the capacity to push to target environments where the ISVs and end-customer have shared-responsibility over the target environment.
...
As Kubernetes has multiple versions and distributions, Replicated offers a compatibility matrix to ensure that ISVs can test their applications in different Kubernetes environments.
...
ISVs can directly push their oci artifacts to the Distr registry and grant access to specific tags to specific customers.
...
Replicated also offers a dedicated registry and also a proxy registry where ISVs can use a registry proxy to perform the registry authentication before pulling the artifacts from the source registry.
...
Whether you are a free plan user or have an enterprise vendor account, Distr provides tailored onboarding support to make sure your application is on-prem ready.
...
If ISVs distributing with Distr have end-customers who do not want to manage the software deployments themselves, ISVs can push, monitor, and manage software directly from the Distr platform itself.
...
The time it takes to implement Distr depends largely on how prepared your software is for on-prem deployment.
...
However, if your software isn't quite "on-prem" ready, it may take longer to realize the full benefits of Distr.
In these cases, we offer hands-on onboarding support and can collaborate with you to improve your overall on-prem readiness.
...
- **Empowering End Customers:** When serving on-prem customers who want full control over their environments,
...
Distr isn’t about prescribing a single "right" way to handle on-prem software delivery.
